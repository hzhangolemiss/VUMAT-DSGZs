...
C
C J2 Mises Plasticity with generalized DSGZ for
...
C      if (if_tc .eq. int(1)) then
C        sigmaY = paraK/(1+paragamma/3)*
C     1           (funcf+(funcq-funcf)*funcr)*funch
C      else if (if_tc .eq. int(-1)) then
C        sigmaY = paraK/(1-paragamma/3)*
C     1           (funcf+(funcq-funcf)*funcr)*funch
C      else if (if_tc .eq. int(0)) then
C        sigmaY = paraK*
C     1           (funcf+(funcq-funcf)*funcr)*funch
C      else
C        sigmaY = paraK*
...
C            if (eqep .eq. 0.0) tempInit = tempOld(k)
...
C
     ...
     1                     yield, eqep, doteqep, temp, if_tc,
     2                     paraK, paragamma,
     3                     paraC1, paraC2, paraC3, paraC4,
     4                     paraa, paraalpha, param)
